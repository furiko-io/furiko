...
func NewInformerWorker(ctrlContext *Context) *InformerWorker {
	...
	// Add event handler for JobConfigs.
...
func (w *InformerWorker) enqueueObject(obj interface{}) {
	...
	if err != nil {
		klog.ErrorS(err, "jobconfigcontroller: keyfunc error", "worker", w.WorkerName(), "obj", obj)
		return
	}
...
func (w *InformerWorker) handleJob(obj interface{}) {
	...
	if err != nil {
		klog.ErrorS(err, "jobconfigcontroller: unable to handle event", "worker", w.WorkerName())
		return
	}

	rjc, err := jobconfig.LookupJobOwner(rj, w.jobconfigInformer.Lister().JobConfigs(rj.Namespace))
	if err != nil {
		klog.ErrorS(err, "jobconfigcontroller: cannot look up jobconfig for job",
			"worker", w.WorkerName(),
